...
class Talk(object):
    ...
    @loggedIn
    def sendMention(self, to, text='', icon='', link='', nama='', mids=[]):
        arrData = ""
        arr = []
        mention = "@kiss "
        if mids == []:
        ...
        self.sendMessage(to, textx, {'AGENT_ICON':icon,'AGENT_LINK':link,'AGENT_NAME':nama,'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
    ...
    @loggedIn
    def sendImage(self, to, path, icon='', link='', nama=''):
        contentFooter = {
           'AGENT_ICON': icon,
           'AGENT_LINK': link,
           'AGENT_NAME': nama,
